...
description: >
  ...
  external sources.
tools: WebSearch, WebFetch, Task, AskUserQuestion, mcp__context7__resolve-library-id, mcp__context7__query-docs
model: opus
---
...
3. **Library & Technology Documentation Research**
   - Use Context7 MCP tools to find library information:
     - First call mcp__context7__resolve-library-id to find the library
     - Then call mcp__context7__query-docs to get up-to-date documentation
   - If Context7 doesn't have the library, use WebFetch to retrieve official documentation
   ...
   - Use AskUserQuestion if you need clarification on which library/approach to research
...
- Prioritize Context7 MCP tools for library documentation (always up-to-date)
- Prioritize official documentation over third-party sources when using WebFetch
...
- Use Task tool to parallelize research across multiple domains
- Use AskUserQuestion when research scope is ambiguous or when choosing between approaches
