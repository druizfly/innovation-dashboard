...
def get_model_display_info(model_name):
    """Map Bedrock ARN to friendly model names with colors."""
    model_mapping = {
        'arn:aws:bedrock:us-east-1:193501891505:application-inference-profile/vm7o7ynuvi8a': {
            'name': 'Haiku',
            'color': '\033[32m'  # Green
        },
        'arn:aws:bedrock:us-east-1:193501891505:application-inference-profile/wtr2o2b9lin3': {
            'name': 'Opus',
            'color': '\033[31m'  # Red
        },
        'arn:aws:bedrock:us-east-1:193501891505:application-inference-profile/8qi7ayvne7iv': {
            'name': 'Sonnet',
            'color': '\033[33m'  # Orange (yellow in terminal)
        }
    }

    # Return mapped info if found, otherwise default
    return model_mapping.get(model_name, {
        'name': model_name,
        'color': '\033[94m'  # Blue (default)
    })
...
def main():
    try:
        ...
        # Extract information
        raw_model_name = data.get('model', {}).get('display_name', 'Claude')
        model_info = get_model_display_info(raw_model_name)
        ...
        # Model display with model-specific color
        reset = "\033[0m"
        model_display = f"{model_color}[{model_name}]{reset}"
